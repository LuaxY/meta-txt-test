...
// npx hardhat run scripts/4-forwaded-eip2771-erc721.ts
...
async function main() {
    ...
    const ERC721Token = await ethers.getContractFactory("MockEIP2771ERC721");
    const erc721Token = await ERC721Token.deploy(minimalForwarder.address); // Trusted Forwarder
    ...
    console.log("npx hardhat verify --contract contracts/MockEIP2771ERC721.sol:MockEIP2771ERC721", erc721Token.address, minimalForwarder.address)
    try {
        await run("verify:verify", {
            address: erc721Token.address,
            contract: "contracts/MockEIP2771ERC721.sol:MockEIP2771ERC721",
